...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---- 宿主保存调用：经 api-gateway 安装的 remote.fileAttachment 服务 ----
		async function saveFileToHost(fileName, b64, sessionId) {
			if (ctxRef === null) throw new Error('插件上下文未就绪')
			const svc = ctxRef.get('remote.fileAttachment')
			if (svc === undefined || typeof svc.save !== 'function') throw new Error('宿主 fileAttachment 远程服务不可用')
			const res = await svc.save(fileName, b64, sessionId)
			if (res === null || typeof res !== 'object') throw new Error('宿主保存无返回')
			if (typeof res.error === 'string' && res.error !== '') throw new Error(res.error)
			if (typeof res.path !== 'string' || res.path === '') throw new Error('宿主保存未返回路径')
			return res
		}
